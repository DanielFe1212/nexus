...
def dashboard_kpi(request):
    ...
    # --- FILTROS INDEPENDIENTES (Para HTML) ---
    ...
    # --- FECHAS PARA TABLAS GENERALES (1, 2, 5, 6) ---
    ...
    for sede in sedes:
        ...
        reporte_sedes.append({
            ...
            'min_p': caida_p,
            'disp_p': round((1 - (caida_p / minutos_mes_total)) * 100, 4) if minutos_mes_total > 0 else 100,
            ...
            'min_s': caida_s,
            'disp_s': round((1 - (caida_s / minutos_mes_total)) * 100, 4) if minutos_mes_total > 0 else 100,
        ...
        if conteo_sedes > 0:
            ...
            caida_total_prov = sum(int((min(ev.fecha_fin or timezone.now(), ultimo_dia_mes) - max(ev.fecha_inicio,
                                   ...
                                   max(ev.fecha_inicio, primer_dia_mes) < min(ev.fecha_fin or timezone.now(),
                                                                              ultimo_dia_mes))
            disp_canal = (1 - (caida_total_prov / min_posibles_global)) * 100 if min_posibles_global > 0 else 100
            reporte_canales.append({
                'nombre': prov.nombre, 'sedes_uso': conteo_sedes, 'min_caida': caida_total_prov,
                'min_posibles': min_posibles_global, 'disp': round(disp_canal, 4), 'cumple': (disp_canal / 100) >= meta
            })

    # --- TABLA 3: MATRIZ HISTÓRICA ---
    meses_historial = []
    meses_nombres = ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Septiembre', 'Octubre',
                     'Noviembre', 'Diciembre']
    for num_mes, nombre_corto in enumerate(meses_nombres, start=1):
        ...
        datos_mes = {'nombre': f"{nombre_corto[:3].upper()} {anio_matriz}", 'valores_prov': []}
        for prov in proveedores:
            ...
            if sedes_uso > 0:
                ...
                evs = Evento.objects.filter(Q(idproveedor=prov) & Q(fecha_inicio__lte=u_dia_m) & (
                ...
                caida = sum(int((min(ev.fecha_fin or timezone.now(), u_dia_m) - max(ev.fecha_inicio,
                            ...
                            ev in evs if max(ev.fecha_inicio, p_dia_m) < min(ev.fecha_fin or timezone.now(), u_dia_m))
    ...
    if sede_obj:
        for numero_mes, nombre_mes in enumerate(meses_nombres, start=1):
            _, ult_dia = calendar.monthrange(anio_hist, numero_mes)
            p_h = timezone.make_aware(datetime(anio_hist, numero_mes, 1), tz)
            u_h = timezone.make_aware(datetime(anio_hist, numero_mes, ult_dia, 23, 59, 59), tz)
            ...
            historico_sede.append({
                'mes': f"{nombre_mes[:3].upper()} {anio_hist}",
    ...
    # --- TABLA 6: GLOBAL PROVEEDORES MPLS ---
    ...
    for prov in proveedores:
        ...
        if conteo_sedes_mpls > 0:
            ...
            caida_total_mpls = sum(int((min(ev.fecha_fin or timezone.now(), ultimo_dia_mes) - max(ev.fecha_inicio,
                                   ...
                                   max(ev.fecha_inicio, primer_dia_mes) < min(ev.fecha_fin or timezone.now(),
                                                                              ultimo_dia_mes))
            disp_canal_mpls = (1 - (caida_total_mpls / min_posibles_mpls)) * 100 if min_posibles_mpls > 0 else 100
    ...
    return render(request, 'dashboard.html', {
        'mes_gen': mes_gen, 'anio_gen': anio_gen, 'mes_nombre': calendar.month_name[mes_gen].upper(),
        'anio_matriz': anio_matriz, 'anio_hist': anio_hist, 'sede_obj': sede_obj,
        'minutos_mes': minutos_mes_total, 'meta_global': meta * 100,
        'reporte': reporte_sedes, 'canales': reporte_canales, 'resumen_historico': meses_historial,
        'lista_proveedores': proveedores, 'sedes': sedes, 'historico_sede': historico_sede,
